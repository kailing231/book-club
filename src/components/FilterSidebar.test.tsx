...
describe('FilterSidebar', () => {
  ...
  it('moves a subject to subjectInclude when its include radio is chosen', async () => {
    ...
    await user.click(within(group).getByRole('radio', { name: 'Include: Science Fiction' }))
  ...
  it('moves a subject to subjectExclude when its exclude radio is chosen', async () => {
    ...
    const group = screen.getByRole('radiogroup', { name: 'Science Fiction' })
    await user.click(within(group).getByRole('radio', { name: 'Exclude: Science Fiction' }))
  ...
  it('clears all filters when reset is pressed', async () => {
    ...
    render(
      <FilterSidebar
        open
        allSubjects={['Science Fiction']}
        filters={{ title: 'dune', authors: 'herbert', subjectInclude: [], subjectExclude: [] }}
        onChange={onChange}
        onClose={() => {}}
      />,
    )

    await user.click(screen.getByRole('button', { name: UI_TEXT.filter.resetAll }))

    expect(onChange).toHaveBeenCalledWith({ title: '', authors: '', subjectInclude: [], subjectExclude: [] })
  })
  ...
  it('closes via the close button', async () => {
    ...
    await user.click(screen.getByRole('button', { name: UI_TEXT.common.close }))
