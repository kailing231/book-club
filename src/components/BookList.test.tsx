...
function displaySelect() {
  return screen.getByRole('combobox', { name: UI_TEXT.list.displayLabel }) as HTMLSelectElement
}
...
describe('BookList row structure', () => {
  ...
  it('left-aligns the title and places authors and subjects below the title row', () => {
    ...
    expect(
      top.querySelector('.book-authors')!.compareDocumentPosition(
        top.querySelector('.book-subjects')!,
      ) & Node.DOCUMENT_POSITION_FOLLOWING,
    ).toBeTruthy()
  })
  ...
  it('keeps the badge tooltip inside the card without clipping text', () => {
    expect(css).toMatch(
      /\.badge\s*::after\s*\{[^}]*white-space:\s*normal/,
    )
    expect(css).toMatch(
      /\.badge\s*::after\s*\{[^}]*max-width:\s*240px/,
    )
  })
})
...
describe('BookList collapse controls', () => {
  ...
  it('expanding a collapsed row restores its comments', () => {
    ...
    fireEvent.click(
      screen.getByRole('button', { name: UI_TEXT.list.commentsHeader(1, true) }),
    )
...
describe('BookList comments', () => {
  it('shows the login note when signed out', () => {
    ...
    expect(
      container.querySelector('.comment-add'),
  ...
  it('keeps the add form visible when the current user already commented', () => {
    ...
    const { container } = renderSignedIn([book({ comments: [own] })], vi.fn(), vi.fn(), vi.fn())
  ...
  it('saves an edited comment via the edit button', () => {
    ...
    renderSignedIn([book({ comments: [own] })], vi.fn(), vi.fn(), onEditComment)
  ...
  it('cancels an edit without saving', () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: UI_TEXT.common.cancel }))
...
describe('BookList covers', () => {
  ...
  it('shows the placeholder image when only a work OLID exists and no cover id', () => {
    const { container } = renderWith([book({ cover_i: null, book_api_id: 'OL1W' })])
...
describe('BookList display and pagination', () => {
  ...
  it('steps back a page with the Previous control', () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: UI_TEXT.list.previous }))
  ...
  it('marks the active page and jumps to a selected numbered page', () => {
    ...
    expect(container.querySelector('.pagination-page--current')).toHaveTextContent(
      '1',
    )
    expect(
      screen.getByRole('button', { name: UI_TEXT.list.pageNumberAria(2) }),
