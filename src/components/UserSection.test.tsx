...
function signedInUser(user: User = { id: 'u1', name: 'Alice', created_at: '' }): User {
  return user
}
...
function setup(overrides: { currentUser?: User | null; onLogout?: () => void } = {}) {
...
describe('UserSection signed out', () => {
  it('keeps the search input enabled', () => {
    setup()
    expect(screen.getByPlaceholderText(UI_TEXT.user.searchPlaceholder)).not.toBeDisabled()
  })

  it('enables Add User once a valid new name is typed', async () => {
    const user = userEvent.setup()
    setup()
    const addButton = screen.getByRole('button', { name: UI_TEXT.user.addUser })
    expect(addButton).toBeDisabled()
    await user.type(screen.getByPlaceholderText(UI_TEXT.user.searchPlaceholder), 'Carol')
    expect(addButton).toBeEnabled()
  })

  it('does not show a Logout button', () => {
    setup()
    expect(screen.queryByRole('button', { name: UI_TEXT.user.logout })).not.toBeInTheDocument()
  })
  ...
  it('lists matching users in the dropdown while typing', async () => {
    const user = userEvent.setup()
    setup()
    await user.type(screen.getByPlaceholderText(UI_TEXT.user.searchPlaceholder), 'Ali')
  ...
  it('adds a new user from the search text', async () => {
    ...
    await user.click(screen.getByRole('button', { name: UI_TEXT.user.addUser }))
...
describe('UserSection signed in', () => {
  it('shows the signed-in note with a Logout button beside it', () => {
    ...
    expect(screen.getByRole('button', { name: UI_TEXT.user.logout })).toBeInTheDocument()
  })
  ...
  it('returns to the signed-out state after logging out', async () => {
    ...
    expect(search).toHaveValue('')
    expect(screen.getByRole('button', { name: UI_TEXT.user.addUser })).toBeDisabled()
  })
})