...
function filters(overrides: Partial<BookFilters> = {}): BookFilters {
  return { title: '', authors: '', subjectInclude: [], subjectExclude: [], ...overrides }
}
...
describe('applyFilters', () => {
  ...
  it('includes only books that have every included subject', () => {
    expect(applyFilters(all, filters({ subjectInclude: ['Adventure'] }))).toEqual([dune, moby])
    expect(
      applyFilters(all, filters({ subjectInclude: ['Science Fiction', 'Adventure'] })),
    ).toEqual([dune])
  })

  it('excludes books that have any excluded subject', () => {
    expect(applyFilters(all, filters({ subjectExclude: ['Adventure'] }))).toEqual([gatsby])
  })

  it('combines title and exclusion filters', () => {
    expect(
      applyFilters(all, filters({ title: 'dick', subjectExclude: ['Science Fiction'] })),
    ).toEqual([moby])
  })
  ...
  it('treats books missing the subjects field as having no subjects', () => {
    ...
    expect(applyFilters([legacy], filters({ subjectInclude: ['Adventure'] }))).toEqual([])
    expect(applyFilters([legacy], filters({ subjectExclude: ['Adventure'] }))).toEqual([legacy])
  })
})
...
describe('sortBooks', () => {
  ...
  it('sorts by read count descending', () => {
    const byRead = [
      ...
      makeBook({ id: 'b', votes: [vote(true, true), vote(false, true), vote(false, true)] }),
      makeBook({ id: 'c', votes: [] }),
    ]
    expect(sortBooks(byRead, 'read', 3).map((b) => b.id)).toEqual(['b', 'a', 'c'])
  })
  ...
  it('sorts by recommendation count descending', () => {
    ...
    expect(sortBooks(byRec, 'recommended', 3).map((b) => b.id)).toEqual(['b', 'a', 'c'])
  })
  ...
  it('sorts by percentRecommended descending (rating default)', () => {
    ...
    expect(sortBooks(byRating, 'rating', 2).map((b) => b.id)).toEqual(['b', 'a', 'c'])
  })
